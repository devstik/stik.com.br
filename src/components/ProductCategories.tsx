                ...
                >
                    {categories.map((cat) => (
                        <motion.div 
                            ...
                            variants={item}
                        >
                            <Link to={`/categorias/${cat.name.toLowerCase().replace(/\s/g, "-")}`}>
                                <div className="w-24 h-24 hover:shadow-xl md:w-32 md:h-32 rounded-full overflow-hidden shadow-md border-2 border-gray-200 mb-2 bg-gray-100 flex items-center justify-center">
                                    <img
                                    src={cat.image}
                                    alt={cat.name}
